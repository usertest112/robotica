...
int colorSensorIzquierdo(tSensors sIzquierdo){
	...
	if(colorIzquierdo > 8)												{colorBlanco = 0;} 		//SE RETORNA 0 -> COLOR BLANCO;
	if(colorIzquierdo >= 4 && colorIzquierdo <= 7){colorBlanco = 2;}	  //SE RETORNA 2 -> COLOR POSIBLE VERDE;
...
int colorSensorDerecho(tSensors sDerecho){
	...
	if(colorDerecho > 9)                      {colorBlanco = 0;}        //SE RETORNA 0 -> COLOR BLANCO;
	if(colorDerecho >= 7 && colorDerecho <= 8){colorBlanco = 2;}        //SE RETORNA 2 -> COLOR POSIBLE VERDE
...
int colorRGBSensorIzquierdo(tSensors sIzquierdo){
	...
	if(getColorReflected(sIzquierdo) < 20){//PREGUNTA SI EL VALOR REFLEJADO ES MENOR QUE 20 (SI NO ES BLANCO)
		...
		if(colorHUE >= 105 && colorHUE <= 125){colorSensor = 3;}else{colorSensor = 1;}//VERIFICA SI EL COLOR ES VERDE O NEGRO
	}else{colorSensor = 2;}
...
int colorRGBSensorDerecho(tSensors sDerecho){
	...
	if(getColorReflected(sDerecho) < 20 ){//PREGUNTA SI EL VALOR REFLEJADO ES MENOR QUE 11 (SI NO ES BLANCO)
		while(tiempoLectura > -1){colorHUE = getColorHue(sDerecho); tiempoLectura--;}//GUARDA LA LECTURA DE COLOR
		if(colorHUE >= 140 && colorHUE <= 165){colorSensor = 3;}
	 else{colorSensor = 1;}//VERIFICA SI EL COLOR ES VERDE O NEGRO
...
task main(){
	...
	seguidorLinea://ETIQUETA DEL SEGUIDOR DE LINEA
		...
		if(distanciaCM <= 5){//SE PREGUNTA SI LA DISTANCIA ES MENOR QUE 5CM, ENTONCES
			...
			if(giroObstaculo == 0){//SI LA VARIABLE giroObstaculo ES 0 ENTONCES LO ESQUIVAR POR LA DERECHA.
				...
				while(getColorReflected(sensorDerecho) > 10){//EL ROBOT GIRA HASTA QUE EL SENSOR DERECHO VEA LA LINEA NEGRA.
					setMotorSpeed(motorIzquierdo, 15);
		...
		if (colorIzq  == 2 ||  colorDer == 2){
			...
			if(colorIzq == 2){//COMPROBACION DE VERDE
				...
				if(colorIzqRGB == 1 || colorIzqRGB == 2){
					if(colorIzqRGB == 1){
						...
						if(colorDerRGB == 1 || colorCenReflected <= 20){
			...
			if(colorDer == 2){ //COMPROBACION DE VERDE
				...
				if(colorDerRGB == 1 || colorDerRGB == 2){
					if(colorDerRGB == 1){
						colorIzqRGB = colorRGBSensorIzquierdo(sensorIzquierdo);
						if(colorIzqRGB == 1 || colorCenReflected <= 20){
							setMotorSyncTime(motorIzquierdo, motorDerecho, 0, 0, 100);
							sleep(275);
							goto seguidorLinea;
						}else{
		...
		if(colorCenReflected > 85){
			stopAllMotors();
			playTone(1000, 3);
		rescate:
			//distanciaCM = getUSDistance(sensorUltrasonido);
			while(getUSDistance(sensorUltrasonido) > 15){
				setMotorSpeed(motorRescate, -60);
				sleep(200);
				setMotorSpeed(motorRescate, 60);
				sleep(200);

	...
	while(true){
		...
		seguidorLinea:
			...
			if(colorIzqReflected <= 10 && colorCenReflected <= 25){
				setMotorSyncTime(motorIzquierdo, motorDerecho, 0, -100, 20);
				sleep (150);
				setMotorSyncTime(motorIzquierdo, motorDerecho, 0, 0,0 );
				if (getColorReflected(sensorCentro)>= 36){
			  forward(1.5, rotations, velocidadAvance);
		   	...
		   	forward(1.5, rotations, velocidadAvance);
				setMotorSyncTime(motorIzquierdo, motorDerecho, 0, 0, 0);
				forward(1.5, rotations, velocidadAvance);
			if(getColorReflected (sensorCentro)){
				 forward(1.5, rotations, velocidadAvance);
			}
			}
